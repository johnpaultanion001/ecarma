...
<div class="mt--6 card m-2">
  <div class="card-header border-0">
    <div class="row align-items-center">
      <div class="col">
        <h3 class="mb-0 text-uppercase" id="titletable">MANAGE SELLERS</h3>
      </div>
    ...
    <table class="table align-items-center table-flush datatable display" cellspacing="0" width="100%">
      ...
      <tbody class="text-uppercase font-weight-bold">
        @foreach($sellers as $seller)
              <tr>
                  <td>
                      <button type="button" name="edit" edit="{{  $seller->id ?? '' }}" class="text-uppercase edit btn btn-info btn-sm"><i class="fas fa-edit"></i></button>
                      <button type="button" name="remove" remove="{{  $seller->id ?? '' }}" class="text-uppercase remove btn btn-danger btn-sm"><i class="fas fa-trash"></i></button>
                  </td>
                  <td>
                      {{  $seller->id ?? '' }}
                  </td>
                  ...
                  <td>
                      {{ $seller->created_at->format('M j , Y h:i A') }}
                  </td>
              </tr>
          @endforeach
      </tbody>
...
$('#myForm').on('submit', function(event){
    ...
    $('.form-control').removeClass('is-invalid')
    var action_url = "{{ route('admin.sellers.store') }}";
    var type = "POST";

    if($('#action').val() == 'Edit'){
        var id = $('#hidden_id').val();
        action_url = "sellers/" + id;
        type = "PUT";
    }
